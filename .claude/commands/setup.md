...
- **Ask** whether to add a `cc` shell alias for the launch flags. If yes: detect the shell rc (`~/.zshrc`, `~/.bashrc`, or `~/.bash_profile` per `$SHELL`), skip if a `cc` alias already exists, else append:
  `alias cc='claude --dangerously-skip-permissions --dangerously-load-development-channels server:channel-relay'`
  and tell them to `source` it.
  - **Flag the trade-off when asking:** this alias permanently runs Claude with `--dangerously-skip-permissions` (no permission prompts) — convenient for the relay flow, but any prompt-injection in a `cc` session then has unrestricted tool access. Let the user opt in knowingly.
...
- If yes:
  ...
  - The AUQ bridge also needs **tmux**, **Claude Code ≥ v2.1.85**, and the bot running on the **same host** (it calls `localhost`). Check `command -v tmux` and warn if missing — the bridge is inert without these, so the `/clear` follow still works either way. See [docs/advanced-hooks.md](../../docs/advanced-hooks.md).
...
2. In any project, launch a session with `cc` (or the full flags).
